...
public class ChatUIFriends implements MessageObserver {
    ...
    class PrivateChatWindow extends JFrame {
        ...
        public PrivateChatWindow(String friendName) {
            ...
            JButton fileListButton = new JButton("文件传输");
        ...
        private void showFileListDialog() {
            JDialog dialog = new JDialog(this, "文件传输记录", true);
            dialog.setLayout(new BorderLayout());
            dialog.setSize(500, 400);
            dialog.setLocationRelativeTo(this);

            // 发送文件列表请求
            client.sendMessage("/filelist " + friendName);
            ...
            // 等待服务器响应的加载提示
            JLabel loadingLabel = new JLabel("正在加载文件列表...");
            fileListPanel.add(loadingLabel);

            // 处理文件列表响应
            MessageObserver fileListObserver = new MessageObserver() {
                @Override
                public void onMessageReceived(String message) {
                    if (message.startsWith("FILELIST:")) {
                        SwingUtilities.invokeLater(() -> {
                            fileListPanel.removeAll();
                            if (message.equals("FILELIST:EMPTY")) {
                                fileListPanel.add(new JLabel("暂无文件记录"));
                            } else {
                                ...
                                for (String fileInfo : files) {
                                    ...
                                    if (parts.length >= 5) {
                                        ...
                                        fileCard.setBorder(BorderFactory.createEtchedBorder());
                                        fileCard.setMaximumSize(new Dimension(450, 60));

                                        JPanel infoPanel = new JPanel(new GridLayout(2, 1));
                                        infoPanel.add(new JLabel(fileName));
                                        infoPanel.add(new JLabel("发送者: " + sender + "  大小: " + fileSize/1024 + "KB"));
                                            ...
                                            if (fileChooser.showSaveDialog(dialog) == JFileChooser.APPROVE_OPTION) {
                                                ...
                                                FileTransferManager.downloadFile(client.getSocket(), fileId, savePath,
                                                        new FileTransferListener() {
                                                            ...
                                                            public void onComplete(String filePath) {
                                                                SwingUtilities.invokeLater(() ->
                                                                        JOptionPane.showMessageDialog(dialog, "文件下载完成!"));
                                                            }
            ...
            client.addObserver(fileListObserver);
